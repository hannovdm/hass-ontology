...
def _node_ref(node: Any) -> dict[str, Any] | None:
    ...
    if properties is None:
        return None
    return {"ha_id": properties.get("ha_id"), "name": properties.get("name")}
...
def _build_result(
    ...
    *,
    ...
    has_dependencies = bool(
        dependencies["automations"] or dependencies["scripts"] or dependencies["scenes"]
    )
    return {
        "scope": scope,
...
def _warnings_for(result: dict[str, Any]) -> list[str]:
    return [] if result["has_dependencies"] else [_NO_DEPENDENCIES_WARNING]
...
async def _analyze_entity(client: MemgraphClient, target: str) -> dict[str, Any]:
    ...
    return build_tool_result(target, RESULT_TYPE_IMPACT_ANALYSIS, result, _warnings_for(result))
...
async def _analyze_device(client: MemgraphClient, target: str) -> dict[str, Any]:
    ...
    entities = [node_properties(row["e"]) for row in entity_rows if row.get("e") is not None]
    dependency_dicts = [await _entity_dependencies(client, e["ha_id"]) for e in entities]
...
async def _analyze_area(client: MemgraphClient, target: str) -> dict[str, Any]:
    ...
    for device in devices:
        device_result = await _analyze_device(client, device["ha_id"])
    ...
    for row in direct_entity_rows:
        ...
        dependency_dicts.append(await _entity_dependencies(client, entity["ha_id"]))
    ...
    result = _build_result(
        ...
        affected_devices=[d.get("ha_id") for d in devices],
        dependencies=merged,
    )
    return build_tool_result(target, RESULT_TYPE_IMPACT_ANALYSIS, result, _warnings_for(result))


async def analyze(client: MemgraphClient, scope: str, target: str) -> dict[str, Any]:
    """Run the bounded-depth impact-analysis traversal for `scope` (FR-013–FR-018)."""
    if scope == IMPACT_SCOPE_ENTITY:
        return await _analyze_entity(client, target)
    if scope == IMPACT_SCOPE_DEVICE:
        return await _analyze_device(client, target)
    if scope == IMPACT_SCOPE_AREA:
        return await _analyze_area(client, target)
